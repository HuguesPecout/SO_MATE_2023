...
institute: "Journées annuelles SO-MATE 2023, Limoges"
...
format:
  revealjs: 
    ...
    footer: "Journées annuelles SO-MATE 2023 - Représentations graphiques avec R"
    scrollable: TRUE
    smaller: TRUE
    overview: TRUE
    chalkboard: FALSE
...
# [I. Représentation graphique en R-base]{.bigtwo}
...
# [II. La révolution `ggplot2`]{.bigtwo}

## [L'avant `ggplot2`]{.bigone}

[Le package `lattice` [@Lattice], sortie en 2001, vise à améliorer les graphiques de base de R en fournissant des valeurs par défaut améliorées et une visualisation simplifiée des relations multivariées.]{.bigtwo} 
...
# [III. Packages thématiques et représentation graphique]{.bigtwo}
...
[Et ses packages d'exploration et de visualisation associés : `FactoInvestigate`, `factoextra`, `Factoshiny`]{.bigtwo}
...
# [Du coté des sciences dures...]{.bigtwo} 
...
[Pour importer, manipuler, traiter et afficher des données géographiques vectorielles.]{.bigtwo}
...
[Pour importer, manipuler, traiter et afficher des données géographiques raster (avec vecteur).]{.bigtwo}
...
[Pour produire des cartes thématiques.]{.bigtwo}
...
[Pour produire des cartes thématiques statique ou intéractive avec la syntaxe ggplot.]{.bigtwo}
...
[Ou encore `ggplot2` & `ggmap` [@R-ggmap]]{.bigtwo}


# [IV. Animation & intéractivité]{.bigone}

## [Représentation animée]{.bigone} 
...
### L'extension `gganimate` [@gganimate]
...
[Exemple personnel `ggplot2` + `gganimate`]{.bigtwo}

![](images/velo.gif){width=75%"}
...
## [Représentation intéractive]{.bigone} 

::: {.bigtwo}
Le langage R, seul, ne permet pas de produire de représentation graphique interactive. Cette fonctionnalité est accessible grâce au langage javascript (+ un navigateur web). Il est possible de créer des [*htmlwidgets*](http://www.htmlwidgets.org), des packages qui permettent de faire un lien avec des librairies javascript...
...
```{=html}
<iframe width="1070" height="540" src="http://www.htmlwidgets.org/" title="htmlwidgets"></iframe>
```
...
[**Quelques exemples :**]{.bigtwo}

:::

\

### Package `leaflet` [@R-leaflet]
...
[Ce package est également compatible avec les graphiques `ggplot2`, qu'il peut rendre intéractif.]{.bigtwo}
...
###  Package `rbokeh` [@rbokeh]



\

###  Package `highcharter` [@highcharter]
...
###  Package `robservable` [@robservable]
...
# [V. Valoriser un graphique fait avec R]{.bigone} 

\

## [Export classique]{.bigone} 
...
[En clic-bouton avec Rstudio :]{.bigtwo}


![](images/export.png){width="70%"}
...
## [Les documents computationnels]{.bigone}

[Rmarkdown & Quarto]{.bigquat}
...
## <img src="images/htmlwidget.png" width="133" style="margin-top:0px;margin-bottom:-5px;margin-right:13px;"/> [http://gallery.htmlwidgets.org/](http://gallery.htmlwidgets.org/)

```{=html}
<iframe width="1070" height="600" src="http://gallery.htmlwidgets.org/" title="ggextension"></iframe>
```
...
::: {.bisbis}
- [**16 couches géographiques**]{.elementR} au format [*shapefile*]{.elementR} (.shp) :
...
[Couche de **points** géoréférencés x 8]{.small2}
...
[Couche de **lignes** géoréférencées x 8]{.small2} 
...
[1. **Représentation de réseau avec `igraph`**]{.bigone}  
[2. **Cartographie avec `mapsf`**]{.bigone}
...
[**Télécharger le projet**](https://gitlab.huma-num.fr/hpecout/somate_igraph/-/raw/master/somate_ateliers_R.zip?ref_type=heads) **Rstudio** (.zip)
...
:::{.center}
<button type="submit" class="btn btn btn-succes">[[ **Consulter le support en plein écran** ](https://elementr.gitpages.huma-num.fr/session_reseau/intro_reseau_igraph/){target="_blank"}]{.bigtwo}</button>
:::
