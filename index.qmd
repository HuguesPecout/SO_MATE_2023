...
[Et ses packages d'exploration et de visualisation associés : `FactoInvestigate`, `factoextra`, `Factoshiny`]{.bigone}
...
# [Du coté des sciences dures...]{.bigone} 
...
Il est possible de crée un GIF ou une vidéo à partir de plusieurs images. L'étendue des possibilités est donc large. 

Plusieurs packages facilite la création de représentations graphiques animées en mettant des fonctions simple d'utilisation à disposition.
...
L'extension ggplot2 : `gganimate` [@gganimate]
...
Le package de cartographie `t_map` [@R-tmap]
...
## [Représentation dynamique ou intéractive]{.bigone} 

Par défaut, le langage R seul ne peut pas produire de représentation interactive. 
Cette caractéristique s'obtient avec le langage javascript + un navigateur web (html).

Il est possible de créer des [*htmlwidgets*](http://www.htmlwidgets.org), des packages qui permettent de faire un lien avec des librairies javascript.
...
# [Valoriser un graphique fait avec R]{.bigone} 
