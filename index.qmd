...
[Travail dans le domaine de la **géographie quantitative**  : collecte, gestion, traitement, enrichissement, analyse (statistique, spatiale, réseau, textuelle...), représentation graphique et cartographie de **données géographiques hétérogénes** (données conventionnelles, d'enquête, big data...).]{.small2}

[Principale mission actuelle : **formation aux outils & méthodes pour les données et protocoles dans les Humanités Numériques**]{.small2}
...
## [Représentation de réseau]{.bigone}
...
###  `ggraph` (extension ggplot2) [@ggraph]
...
## [Représentation de séquence]{.bigone}
...
library(wordcloud2) 
library(wordcloud) 
# données en entrée (6 premières lignes)
head(demoFreq)
...
Mais aussi `ggwordcloud` et `ggwordcloud2``...
...
Réseau des relais de poste à 5 dates : 1632, 1708, 1733, 1758, 1783, 1810, 1833
...
16 couches géographiques (données géoréférencées) :

- 8 couches de points (relais de poste). 1 pour chaque date. format shapefile.   
- 8 couches de lignes (route postales). 1 pour chaque date. format shapefile.   
